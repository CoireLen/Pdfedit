...
class ListWidget(QListWidget):
    def __init__(self, parent: Optional[QWidget]=None) -> None:
        ...
        # 拖拽设置
        self.SetMode()
        self.setIconSize(QSize(int(400*main["self"].zoomsize),int(500*main["self"].zoomsize)))
...
class Window(QWidget):
    ...
    def OnClickToolbarButton(self,o):
        ...
        if o.text()=="排序":
            ...
            self.listWidget.setIconSize(QSize(int(400*self.zoomsize),int(500*self.zoomsize)))
        if o.text()=="阅读":
    ...
    def OnReSave(self):
        if self.doc != None:
            ...
            savefile=QFileDialog(self,"选择PDF文件","","PDF 文件(*.pdf)").getSaveFileName()[0]
            savedoc=fitz.open()
            if len(select)==0:
                for i in range(0,self.listWidget.count()):
                    itm=self.listWidget.item(i)
                    savedoc.insert_pdf(self.doc,int(itm.text())-1,int(itm.text())-1,-1)
            else:
                list=[]
                for itm in select:
    ...
    def OnInsterPage(self):
        if self.doc != None:
            self.insterfile=QFileDialog(self,"选择PDF文件","","PDF 文件(*.pdf)").getOpenFileName()[0]
            self.DoInster()
    def DoInster(self):
    ...
    def OnRotatePage(self):
        if self.doc != None:
            ...
            for itm in select:
                ...
                self.lwitem[int(itm.text())-1].setIcon(QIcon(QPixmap.fromImage(qtimg)))
    ...
    def OnOpenFile(self):
        self.openfile=QFileDialog(self,"选择PDF文件","","PDF 文件(*.pdf)").getOpenFileName()[0]
        if self.openfile != "" :
            self.lwitem.clear()
            self.listWidget.clear()
            print(self.openfile)
